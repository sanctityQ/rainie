...
class DTSAspect {
  ...
  def getParamterAnnotationValue(method: Method,dtsAnName: String):Map[String,String]={
    ...
    logger.info("DTSAspect.getParamterAnnotationValue end " + JsonUtil.toJson(map) + ",methodName=" + method.getName)
    map
  }
}
...
class ActivityAspect extends DTSAspect{
  ...
  def doAround(joinPoint: ProceedingJoinPoint):Object ={
    ...
    val bizId = {
      if (txAnValue.value.length != 0) {
        ExtendedBeanUtils.getProperty(args(txAnValue.index), txAnValue.value)
      } else {
        args(txAnValue.index).toString
      }
    }
    //事务开启
...
class ActionAspect extends DTSAspect{
  ...
  def doAround(joinPoint: ProceedingJoinPoint):Object ={
    ...
    val args = joinPoint.getArgs()
    val idempotency ={
      if(actionAnValue.value.length != 0)
        ExtendedBeanUtils.getProperty(args(actionAnValue.index),actionAnValue.value)
      else
        args(actionAnValue.index).toString
    }
